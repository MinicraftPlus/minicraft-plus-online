...
public class MinicraftServer extends Thread implements MinicraftProtocol {
	...
	boolean verifyPlayer(String username, String token) {
		HttpResponse<JsonNode> response = null;
		
		try {
			response = Unirest.post(MultiplayerDisplay.apiDomain+"/verify-token")
				.field("name", username)
				.field("token", token)
				.asJson();
		...
		if(response != null) {
			...
			switch(json.getString("status")) {
				case "error":
					return false;
				case "success":
					if(Game.debug) System.out.println("successfully received token verification from playminicraft server");
	...
	boolean parsePacket(MinicraftServerThread serverThread, InputType inType, String alldata) {
		...
		switch(inType) {
			...
			case LOGIN:
				...
				try {
					Version clientVersion = new Version(data[2]);
					// check version; send back invalid if they don't match.
					if (Game.VERSION.compareTo(clientVersion) != 0) {
						serverThread.sendError("Wrong game version; need " + Game.VERSION.toString());
						return false;
					}
				...
				String token = data[1];
				//Check token here.
				if (!verifyPlayer(username, token)) {
