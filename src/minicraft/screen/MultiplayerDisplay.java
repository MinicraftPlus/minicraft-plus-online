...
public class MultiplayerDisplay extends Display {
	...
	public static void setCustomClients() {
		if (!customClientSet) {
			try {
				Unirest.setHttpClient(HttpClients.custom().setSSLContext(getSSLC()).build());
				Unirest.setAsyncHttpClient(HttpAsyncClients.custom().setSSLContext(getSSLC()).build());
			} catch (Exception e) {
				e.printStackTrace();
				return;
			}
	...
	private static SSLContext getSSLC() throws Exception {
		try {
			...
			String caCertText = "-----BEGIN CERTIFICATE-----\\nQsGVGloahq/9SpglGbag\n-----END CERTIFICATE-----";
			InputStream caCertStream = new ByteArrayInputStream(caCertText.getBytes());
			
			CertificateFactory x509cf = CertificateFactory.getInstance("X.509");
			X509Certificate EVScaCert = (X509Certificate)x509cf.generateCertificate(caCertStream);
			KeyStore EVSks = KeyStore.getInstance(KeyStore.getDefaultType());
			EVSks.load(null); // You don't need the KeyStore instance to come from a file.
			EVSks.setCertificateEntry("CA", EVScaCert);
			caCertStream.close();
			
			tmf = TrustManagerFactory.getInstance(TrustManagerFactory.getDefaultAlgorithm());
			tmf.init(EVSks);
			
			X509TrustManager EVSTm = null;
			for (TrustManager tm : tmf.getTrustManagers()) {
			    if (tm instanceof X509TrustManager) {
			        EVSTm = (X509TrustManager) tm;
			        break;
			    }
			}
			
			final X509TrustManager finalDefaultTm = defaultTm;
			final X509TrustManager finalMyTm = EVSTm;
	...
	public MultiplayerDisplay(String ipAddress) {
		...
		contactAccountServer(() -> {
			if(curState == State.ENTERIP) { // login was automatic
				...
				Game.client = new MinicraftClient(savedUsername, savedToken, this, ipAddress);
			} else
		...
		Unirest.get(domain).asBinaryAsync(new Callback<InputStream>() {
			...
			public void completed(HttpResponse<InputStream> httpResponse) {
				...
				if(savedUUID.length() > 0) {
					...
					if (Game.debug) System.out.println("fetching username for uuid: " + savedUUID);
					fetchName(savedUUID);
				}
		...
		switch(curState) {
			...
			case ENTERIP:
				...
				if(input.getKey("select").clicked) {
					...
					new Thread(() -> {
						Game.client = new MinicraftClient(savedUsername, savedToken, this, typing, connectTimeout.getValue()*1000); // typing = ipAddress
					...
					savedUUID = "";
					savedUsername = "";
					try {Unirest.post(apiDomain+"/logout").field("token", savedToken).asBinary();} catch (UnirestException e) {}
	...
	private void login(String email, String password) {
        ...
        passwordHash = hexString + "";
		} catch (NoSuchAlgorithmException e) { /*It won't happen unless client is corrupt*/ }
